...
class SparqlAlgebra {
  ...
  def +(that: SparqlAlgebra) : SparqlAlgebra = {
    this match {
  ...
  def display( logiceNode: SparqlAlgebra , marge : Int ) : String = {
    val stepMarge = 1
...
case class BGP(lnodes : Seq[RdfNode]) extends SparqlAlgebra
case class OR(lbgp   : Seq[SparqlAlgebra]) extends SparqlAlgebra
case class AND(lbgp : Seq[SparqlAlgebra]) extends SparqlAlgebra
