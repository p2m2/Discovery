...
//, config : StatementConfiguration
    ...
    l match {
      case BgpGroupe(lnodes) => {
        ...
        val lbgpBySource  = lSourcesNodes.map( sn => sn.sources )
                       ...
                       .distinct
                       .map( source =>
                         (source -> lSourcesNodes
                                     .filter( _.sources contains source )
                                     .map( sn => r.getRdfNode(sn.refNode).map( a => a.duplicateWithoutChildren()).getOrElse(null)  )))
  ...
  def cleanPlan(plan : SparqlGroupNode) : SparqlGroupNode = {
    debug("clean plan :"+plan.toString)
...

    {
      ...
      case a : AndGroupe  => {
        ...
        if ( bgpSet.lnodes.length>0 && andSet.lbgp.length>0 && orSet.lbgp.length>0) {
          ...
          error("non traité.....")
          error("bgpSet.lnodes.length:"+bgpSet.lnodes.length.toString)
          error("andSet.lbgp.length:"+andSet.lbgp.length.toString)
          error("orSet.lbgp.length:"+orSet.lbgp.length.toString)
          BgpGroupe(List())
        }
      }
